...
RUN dnf -y update && dnf -y install http://rpms.remirepo.net/fedora/remi-release-38.rpm \
...
RUN dnf -y module enable postgresql:15 && dnf -y install postgresql postgresql-server postgresql-contrib \
    ...
    && cd /usr/local && wget https://golang.google.cn/dl/go1.20.6.linux-amd64.tar.gz \
        && tar -xzvf go1.20.6.linux-amd64.tar.gz && rm go1.20.6.linux-amd64.tar.gz
