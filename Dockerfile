...
RUN dnf -y update && dnf -y install http://rpms.remirepo.net/fedora/remi-release-41.rpm && dnf -y install dnf-plugins-core  \
...
RUN dnf -y install postgresql postgresql-server postgresql-contrib \
    ...
    && cd /usr/local && wget https://golang.google.cn/dl/go1.23.4.linux-amd64.tar.gz \
        && tar -xzvf go1.23.4.linux-amd64.tar.gz && rm go1.23.4.linux-amd64.tar.gz
